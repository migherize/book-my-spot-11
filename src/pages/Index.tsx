      ...
      <div className="min-h-screen bg-background">
        ...
        <section className="relative overflow-hidden py-20 md:py-32">
          ...
          <motion.div className="container relative mx-auto px-4 text-center" variants={stagger} initial="initial" animate="animate">
            <motion.h1 variants={fadeUp} className="font-display text-4xl font-extrabold tracking-tight text-foreground md:text-6xl">
              Agenda tu cita<br /><span className="text-primary">en minutos</span>
            </motion.h1>
            <motion.p variants={fadeUp} className="mx-auto mt-4 max-w-lg text-lg text-muted-foreground">
              Encuentra profesionales verificados de salud, belleza y bienestar. Reserva online fácil, rápido y seguro.
            </motion.p>
            <motion.div variants={fadeUp} className="mx-auto mt-8 flex max-w-xl items-center gap-2 rounded-xl border border-border bg-card p-2 shadow-lg">
        ...
        <section className="py-16">
          <div className="container mx-auto px-4">
            <motion.h2 initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }} className="font-display text-2xl font-bold text-center mb-8">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              {categories.map((cat, i) => {
                ...
                return (
                  <motion.button key={cat.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.1 }} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }} onClick={() => navigate(`/category/${cat.id}`)} className={`group flex flex-col items-center gap-3 rounded-xl border border-border p-8 transition-colors ${categoryColors[cat.type] ?? ""}`}>
        ...
        <section className="py-16 bg-muted/50">
          <div className="container mx-auto px-4">
            <div className="flex items-center justify-between mb-8">
              <motion.h2 initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="font-display text-2xl font-bold">
        ...
        <section className="py-16">
          <div className="container mx-auto px-4 text-center">
            <motion.h2 initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="font-display text-2xl font-bold mb-12">
              ¿Cómo funciona?
            </motion.h2>
            <div className="grid gap-8 md:grid-cols-3">
              {steps.map((step, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.15 }} className="flex flex-col items-center gap-4">
                  <motion.div whileHover={{ scale: 1.1, rotate: 5 }} transition={{ type: "spring", stiffness: 300 }} className="flex h-16 w-16 items-center justify-center rounded-2xl bg-primary/10 text-primary">
